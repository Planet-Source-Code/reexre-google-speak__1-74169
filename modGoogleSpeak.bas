...
Public Function GoogleSpeakLess100chars(ByVal sText As String, Optional ByVal Language As String = "es", Optional ByVal bDoEvents As Boolean = True) As Boolean
    ...
    sTempPath = App.Path & "\TempMP3.MP3"
...
Private Function GoogleSpeakCreateMP3(Number As Long, ByVal sText As String, Optional ByVal Language As String = "es", Optional ByVal bDoEvents As Boolean = True) As Boolean
    ...
    sTempPath = App.Path & "\TempMP3" & Format(Number, "000") & ".MP3"

    URLDownloadToFile 0&, "http://translate.google.com/translate_tts?tl=" & Language & "&q=" & sText, sTempPath, 0&, 0&


End Function

'reexre:
Public Sub SpeekMoreThan100(sText As String, Lang As String)
    Dim S          As String
    Dim SS()       As String
    Dim NS         As Long
    Dim M          As String
    ...
    Else
        Form1.PB.Max = NS
        For I = 1 To NS
            Form1.Caption = "creating portion  " & I & " / " & NS
...
Private Function JoinMP3(N As Long) As String
    ...
    Form1.Caption = "Joining MP3s...."

    For I = 1 To N
        Names(I) = App.Path & "\TempMP3" & Format(I, "000") & ".MP3"
    Next
...
Private Sub PlayMP3(FN As String)  ', Optional bDoEvents As Boolean = True)

    Dim ML         As String
    Dim FileLength As Long

    Form1.Caption = "Speaking... " & FN

    If mciSendString("open " & Chr$(34) & FN & Chr$(34) & " type MpegVideo" & " alias myfile", 0&, 0&, 0&) = 0 Then
        ML = String(30, 0)
        Call mciSendString("status myfile length ", ML, 30, 0&)
        FileLength = Val(ML)
        Form1.PB.Max = FileLength \ 10
        If FileLength Then
            If mciSendString("play myFile", 0&, 0&, 0&) = 0 Then
                Do While mciSendString("status myfile position ", ML, 30, 0&) = 0
